...
def build_artifacts(
    *,
    ...
    summary_content = {
        ...
        "historical_symbols_discovered": len({row["symbol"] for row in source_rows if row["interval"] == "1d"}),
        "excluded_invalid_daily_rows": sum(len(row.get("excluded_invalid_daily_rows", [])) for row in source_rows),

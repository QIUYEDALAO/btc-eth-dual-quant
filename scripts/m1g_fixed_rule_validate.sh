...
report_guard() { grep -qE '^- Status: frozen_pre_implementation$' reports/m1/M1G_FIXED_RULE_CONTRACT.md && grep -qE '^- Parameter search used: no$' reports/m1/M1G_FIXED_RULE_CONTRACT.md && grep -qE '^- Backtest executed: no$' reports/m1/M1G_FIXED_RULE_CONTRACT.md && grep -qE '^- OOS opened: no$' reports/m1/M1G_FIXED_RULE_CONTRACT.md; }
...
run_check "M1G paper validation" bash scripts/m1g_paper_validate.sh
run_check "M1G fixed rule tests" "$PY_CMD" -m unittest tests/test_m1g_fixed_rule.py -v
run_check "M1G fixed rule check" "$PY_CMD" scripts/m1g_fixed_rule_check.py
run_check "compileall" "$PY_CMD" -m compileall src scripts
run_check "M1G fixed rule report gate" report_guard
...
echo; echo "M1G fixed rule validation summary"; for result in "${RESULTS[@]}"; do echo "- $result"; done; echo "PASS=$PASS_COUNT FAIL=$FAIL_COUNT"; [[ "$FAIL_COUNT" -eq 0 ]]

...
report_guard() { grep -qE '^- Status: frozen_before_is_result$' reports/m1/M1G_IS_VALIDATION_PROTOCOL.md && grep -qE '^- IS performance run executed: no$' reports/m1/M1G_IS_VALIDATION_PROTOCOL.md && grep -qE '^- OOS opened: no$' reports/m1/M1G_IS_VALIDATION_PROTOCOL.md; }
...
run_check "M1G implementation validation" bash scripts/m1g_implementation_validate.sh
run_check "M1G IS protocol tests" "$PY_CMD" -m unittest tests.test_m1g_is_protocol -v
run_check "M1G IS protocol check" "$PY_CMD" scripts/m1g_is_protocol_check.py
run_check "compileall" "$PY_CMD" -m compileall src scripts
run_check "protocol report gate" report_guard
...
echo; echo "M1G IS protocol validation summary"; for result in "${RESULTS[@]}"; do echo "- $result"; done; echo "PASS=$PASS_COUNT FAIL=$FAIL_COUNT"; [[ "$FAIL_COUNT" -eq 0 ]]

...
DECISION = ROOT / "config/u25_design_authorization_v1.json"
FAILURE = ROOT / "reports/m1/evidence/u24_cross_sectional_paper_observation/run_manifest.json"
PRIOR = ROOT / "config/u24_design_authorization_v1.json"
AUDIT = ROOT / "reports/m0/evidence/liquid_universe_v4_adr0015_independent_audit/audit_summary.json"
EXPECTED = "577bd189120bb5dc76a79141cc1213b541e19487516bb8c0b7c65a3477606771"
...
def validate(decision: Mapping[str, Any], failure: Mapping[str, Any], prior: Mapping[str, Any], audit: Mapping[str, Any]) -> list[str]:
    ...
    if bindings.get("u24_run_content_hash") != failure.get("run_content_hash") or failure.get("status") != "failed_feasibility":
        findings.append("U-24 result")
    if failure.get("oos_opened") is not False or failure.get("formal_returns_computed") is not False or failure.get("second_run_executed") is not False:
        findings.append("U-24 isolation")
    if (
        bindings.get("u24_protocol_content_hash") != failure.get("protocol_content_hash")
        or bindings.get("u24_design_authorization_hash") != prior.get("content_hash")
    ...
    required = (
        "u25_design_authorized",
        "independent_economic_rationale_required",
        "u04_through_u24_outcome_direction_threshold_rule_censor_or_failure_reuse_prohibited",
        "u24_negative_result_inversion_repair_relabeling_or_repackaging_prohibited",
    ...
    if [key for key, value in decision.get("authorizations", {}).items() if value] != ["u25_hypothesis_design"]:
...
def main() -> int:
    ...
    print("u25_design_authorization_check " + ("PASS" if not findings else "FAIL") + f" hash={EXPECTED}")

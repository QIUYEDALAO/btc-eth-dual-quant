...
report_guard() {
  local report=reports/m1/M1E_IS_DATA_ISOLATION_REPORT.md
  grep -qE '^- Status: pass$' "$report" &&
  grep -qE '^- Candidate evaluated: no$' "$report" &&
  grep -qE '^- Strategy returns computed: no$' "$report" &&
  grep -qE '^- OOS OHLC parsed: no$' "$report" &&
  grep -qE '^- OOS opened: no$' "$report" &&
  grep -qE '^- M1E-06 paper diagnostics authorized: yes$' "$report" &&
  [[ "$(grep -cE '^\| (BTCUSDT|ETHUSDT):(1h|4h) \|' "$report")" -eq 4 ]]
}
...
run_check "M1E IS design validation" bash scripts/m1e_is_design_validate.sh
run_check "M1E isolator tests" "$PY_CMD" -m unittest tests/test_m1e_isolator.py -v
run_check "compileall" "$PY_CMD" -m compileall src scripts
run_check "M1E isolation report gate" report_guard
...
echo
echo "M1E isolator validation summary"

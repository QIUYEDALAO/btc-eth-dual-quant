...
run_check "M1E isolator validation" bash scripts/m1e_isolator_validate.sh
run_check "M1E paper tests" "$PY_CMD" -m unittest tests/test_m1e_paper.py -v
run_check "M1E paper check" "$PY_CMD" scripts/m1e_paper_check.py
...
echo; echo "M1E paper validation summary"; for result in "${RESULTS[@]}"; do echo "- $result"; done; echo "PASS=$PASS_COUNT FAIL=$FAIL_COUNT"; [[ "$FAIL_COUNT" -eq 0 ]]

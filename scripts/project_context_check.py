...
def main() -> int:
    ...
    for required in ("No strategy is eligible for M2", "Do not enter M2"):
    ...
    for required in ("M1B Failed Validation Merged", "Post-M1B Review Started"):
    ...
    if not any("diagnostics" in item for item in allowed_next_work):
        failures.append("PROJECT_STATE.yaml allowed_next_work must include diagnostics")
    if not any("Design review" in item for item in allowed_next_work):

...
def validate(decision: Mapping[str, Any], failure: Mapping[str, Any], prior: Mapping[str, Any], audit: Mapping[str, Any]) -> list[str]:
    ...
    if bindings.get("u19_run_content_hash") != failure.get("run_content_hash") or failure.get("status") != "failed_feasibility":
        findings.append("U-19 result binding")
    if failure.get("oos_opened") is not False or failure.get("formal_returns_computed") is not False or failure.get("second_run_executed") is not False:
        findings.append("U-19 isolation")
    if bindings.get("u19_protocol_content_hash") != failure.get("protocol_content_hash") or bindings.get("u19_design_authorization_hash") != prior.get("content_hash") or bindings.get("adr0015_audit_summary_hash") != audit.get("audit_summary_hash") or bindings.get("v4_artifact_set_hash") != audit.get("independent_artifact_set_hash") or bindings.get("source_freeze_hash") != audit.get("diagnostics", {}).get("source_freeze_content_hash"):
        findings.append("authority")
    required = (
        "u20_design_authorized",
        "independent_economic_rationale_required",
        "u04_through_u19_outcome_direction_threshold_rule_censor_or_failure_reuse_prohibited",
        "u19_negative_result_inversion_repair_relabeling_or_repackaging_prohibited",
    ...
    if [key for key, value in decision.get("authorizations", {}).items() if value] != ["u20_hypothesis_design"]:
...
def main() -> int:
    ...
    print("u20_design_authorization_check " + ("PASS" if not findings else "FAIL") + f" hash={EXPECTED}")

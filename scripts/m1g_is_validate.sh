...
run_check "M1G IS protocol validation" bash scripts/m1g_is_protocol_validate.sh
run_check "M1G IS tests" "$PY_CMD" -m unittest tests.test_m1g_is_validation -v
run_check "M1G IS evidence check" "$PY_CMD" scripts/m1g_is_check.py
...
echo; echo "M1G IS validation summary"; for result in "${RESULTS[@]}"; do echo "- $result"; done; echo "PASS=$PASS_COUNT FAIL=$FAIL_COUNT"; [[ "$FAIL_COUNT" -eq 0 ]]

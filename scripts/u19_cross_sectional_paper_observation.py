#!/usr/bin/env python3
"""Execute the unique frozen U-19 sealed-IS Paper observation."""
...
from btc_eth_dual_quant.data.u19_volatility_of_volatility import VolatilityOfVolatilityCandidatePath, evaluate_decision
...
PROTOCOL_TARGET = "8b8b03f0869463c046668e28fbb22618383addf4"
PROTOCOL_PATH = "config/u19_cross_sectional_paper_protocol_v1.json"
QUALIFICATION_HASH = "62b7ac38cc1eb9ab89d2c2af8571181ef2f06a2f3873d70f61c917d3c23db499"
...
def select_events(
    ...
    accounting = {
        ...
        "zero_base_variability_or_threshold_rank_failed": 0,
        "candidate_events": 0,
    }
    decision = start
    with localcontext() as context:
        ...
        while decision < end:
            ...
            history_starts = range(decision - 336 * ONE_HOUR_MS, decision, ONE_HOUR_MS)
            ...
            rows: list[VolatilityOfVolatilityCandidatePath] = []
            for symbol in symbols:
                peers = tuple(peer for peer in symbols if peer != symbol)
                common = tuple(median([returns[peer][index] for peer in peers]) for index in range(336))
                rows.append(
                    VolatilityOfVolatilityCandidatePath(
                        decision,
                        symbol,
            ...
            selected = evaluate_decision(rows)
            if selected is None:
                accounting["zero_base_variability_or_threshold_rank_failed"] += 1
                decision += FOUR_HOURS_MS
                continue
            ...
            core = {
                ...
                "minimum_two_half_normalized_volatility_of_volatility": selected["minimum_normalized_volatility_of_volatility"],
                "maximum_two_half_normalized_volatility_of_volatility": selected["maximum_normalized_volatility_of_volatility"],
            }
            ...
            paths.append(
                {
                    ...
                    "relative_volatility_of_volatility_risk_premium": {str(hour): decimal_text(relative[hour * 12 - 1]) for hour in HORIZONS},
...
def evaluate_gates(paths: Sequence[Mapping[str, Any]], mismatch: int) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    relative = [Decimal(str(row["relative_volatility_of_volatility_risk_premium"]["24"])) for row in paths]
    ...
    metrics = {
        ...
        "median_24h_relative_volatility_of_volatility_risk_premium": decimal_text(relative_median) if relative_median.is_finite() else None,
        "median_24h_candidate_absolute_close_displacement": decimal_text(absolute_median) if absolute_median.is_finite() else None,
        "fraction_complete_episodes_with_positive_24h_relative_volatility_of_volatility_risk_premium": decimal_text(positive) if positive.is_finite() else None,
    ...
    checks = {
        ...
        "median_24h_relative_volatility_of_volatility_risk_premium": relative_median.is_finite() and relative_median >= Decimal("0.0180"),
        "median_24h_candidate_absolute_close_displacement": absolute_median.is_finite() and absolute_median >= Decimal("0.0180"),
        "fraction_positive_24h_relative_volatility_of_volatility_risk_premium": positive.is_finite() and positive >= Decimal("0.60"),
...
def wrap(name: str, content: Any) -> dict[str, Any]:
    document = {"schema_version": 1, "manifest_type": f"u19_{name}_manifest", "content": content}
...
def run_order(raw_root: Path, protocol: Mapping[str, Any], qualification: Mapping[str, Any], order: str) -> dict[str, Any]:
    ...
    closes = build_hourly_closes(raw_root=raw_root, membership=membership, masked=masked, start=start - 336 * ONE_HOUR_MS, end=end, order=order)
...
def execute(raw_root: Path) -> dict[str, Any]:
    ...
    qualification = load_json(ROOT / "reports/m1/evidence/u19_cross_sectional_data_qualification_v1.json")
    ...
    summary = {
        "schema_version": 1,
        "run_id": "U19-06-SEALED-IS-PAPER-OBSERVATION-V1",
        "status": status,
...
def render(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# U-19 Sealed-IS Paper Observation",
        "",
        ...
        *[f"- {key}: `{str(value).lower()}`" for key, value in summary["paper_gate_checks"].items()],
        ...
        f"- Median 24h relative volatility-of-volatility risk premium: `{metrics['median_24h_relative_volatility_of_volatility_risk_premium']}`",
        f"- Median 24h absolute displacement: `{metrics['median_24h_candidate_absolute_close_displacement']}`",
        f"- Positive relative-premium fraction: `{metrics['fraction_complete_episodes_with_positive_24h_relative_volatility_of_volatility_risk_premium']}`",
        "",
        "## Isolation",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--evidence-dir", type=Path, default=ROOT / "reports/m1/evidence/u19_cross_sectional_paper_observation")
    parser.add_argument("--report", type=Path, default=ROOT / "reports/m1/U19_CROSS_SECTIONAL_PAPER_OBSERVATION.md")
    args = parser.parse_args()
    if (args.evidence_dir / "run_manifest.json").exists():
        print("U-19 is closed after its unique Paper observation; a second result-bearing run is not authorized.")
        return 2

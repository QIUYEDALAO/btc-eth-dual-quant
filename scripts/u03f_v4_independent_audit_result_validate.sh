...
run_check "frozen protocol" "$PY_CMD" scripts/u03f_v4_audit_protocol_check.py
...
run_check "real-audit unit tests" "$PY_CMD" -m unittest tests.test_u03f_v4_independent_audit_run -v
run_check "committed audit evidence" "$PY_CMD" scripts/u03f_v4_independent_audit_result_check.py
...
echo; echo "U-03F V4 independent audit result validation summary"; for result in "${RESULTS[@]}"; do echo "- $result"; done; echo "PASS=$PASS_COUNT FAIL=$FAIL_COUNT"; [[ "$FAIL_COUNT" -eq 0 ]]

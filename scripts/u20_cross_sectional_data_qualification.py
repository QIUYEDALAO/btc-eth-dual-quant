#!/usr/bin/env python3
"""Run U-20 result-free source qualification, structural ceiling and complexity Gate."""
...
from btc_eth_dual_quant.data.u20_negative_coskewness import NegativeCoskewnessCandidatePath, evaluate_stream
...
CONFIG = ROOT / "config/u20_cross_sectional_data_qualification_v1.json"
...
COMMON_HALF = tuple(((index % 7) - 3) * 0.002 for index in range(168))
NEGATIVE_ADJUSTED_HALF = tuple(-50.0 * value * value for value in COMMON_HALF)
NEUTRAL_ADJUSTED_HALF = tuple(0.001 if index % 2 else -0.001 for index in range(168))


def candidate_half(adjusted: tuple[float, ...]) -> tuple[float, ...]:
    return tuple(common + residual for common, residual in zip(COMMON_HALF, adjusted))


NEGATIVE_HISTORY = candidate_half(NEGATIVE_ADJUSTED_HALF) * 2
NEUTRAL_HISTORY = candidate_half(NEUTRAL_ADJUSTED_HALF) * 2
COMMON = COMMON_HALF * 2
PATH = (0.002, 0.004, 0.007, 0.011, 0.015, 0.020)


def synthetic_rows(total: int, active_members: int = 15) -> Iterator[NegativeCoskewnessCandidatePath]:
    histories = tuple(NEGATIVE_HISTORY if symbol_index < 4 else NEUTRAL_HISTORY for symbol_index in range(active_members))
    for index in range(total):
        symbol_index = index % active_members
        decision_index = index // active_members
        yield NegativeCoskewnessCandidatePath(
...
def complexity(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"status": "pass", "passes": passes, "same_code_path": contract["same_common_adjustment_coskewness_event_and_path_code_path"], "market_outcomes_read": 0, "synthetic_output_hash": digest}
...
def qualify(raw_root: Path, config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    result = {
        ...
        "isolation": {
            ...
            "common_adjusted_return_rows_generated": 0,
            "coskewness_statistic_rows_generated": 0,
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "reports/m1/evidence/u20_cross_sectional_data_qualification_v1.json")
    args = parser.parse_args()
    result = qualify(args.raw_root, load_json(CONFIG))
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, sort_keys=True, separators=(",", ":")) + "\n")
    print(f"U-20 qualification PASS ceiling={result['counts']['maximum_independent_theoretical_24h_episodes']} hash={result['qualification_content_hash']}")
    return 0

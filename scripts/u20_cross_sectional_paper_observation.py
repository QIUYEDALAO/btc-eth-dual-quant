#!/usr/bin/env python3
"""Execute the unique frozen U-20 sealed-IS Paper observation."""
...
from btc_eth_dual_quant.data.u20_negative_coskewness import NegativeCoskewnessCandidatePath, evaluate_decision
...
PROTOCOL_TARGET = "6a2207c05c7045e82b47f9685c01a5c2d0b30755"
PROTOCOL_PATH = "config/u20_cross_sectional_paper_protocol_v1.json"
QUALIFICATION_HASH = "de61add6f8ef2a22f62166c14aeb4c30565cb8b3003e6395909fa7b7a85e2368"
...
def select_events(
    ...
    accounting = {
        ...
        "zero_scale_or_coskewness_threshold_rank_failed": 0,
        "candidate_events": 0,
    }
    decision = start
    with localcontext() as context:
        ...
        while decision < end:
            ...
            rows: list[NegativeCoskewnessCandidatePath] = []
            for symbol in symbols:
                ...
                rows.append(
                    NegativeCoskewnessCandidatePath(
                        decision,
                        symbol,
            ...
            if selected is None:
                accounting["zero_scale_or_coskewness_threshold_rank_failed"] += 1
                decision += FOUR_HOURS_MS
                continue
            ...
            core = {
                ...
                "maximum_two_half_coskewness": selected["maximum_half_coskewness"],
                "minimum_two_half_coskewness": selected["minimum_half_coskewness"],
            }
            ...
            paths.append(
                {
                    ...
                    "relative_negative_coskewness_risk_premium": {str(hour): decimal_text(relative[hour * 12 - 1]) for hour in HORIZONS},
...
def evaluate_gates(paths: Sequence[Mapping[str, Any]], mismatch: int) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    relative = [Decimal(str(row["relative_negative_coskewness_risk_premium"]["24"])) for row in paths]
    ...
    metrics = {
        ...
        "median_24h_relative_negative_coskewness_risk_premium": decimal_text(relative_median) if relative_median.is_finite() else None,
        "median_24h_candidate_absolute_close_displacement": decimal_text(absolute_median) if absolute_median.is_finite() else None,
        "fraction_complete_episodes_with_positive_24h_relative_negative_coskewness_risk_premium": decimal_text(positive) if positive.is_finite() else None,
    ...
    checks = {
        ...
        "median_24h_relative_negative_coskewness_risk_premium": relative_median.is_finite() and relative_median >= Decimal("0.0180"),
        "median_24h_candidate_absolute_close_displacement": absolute_median.is_finite() and absolute_median >= Decimal("0.0180"),
        "fraction_positive_24h_relative_negative_coskewness_risk_premium": positive.is_finite() and positive >= Decimal("0.60"),
...
def wrap(name: str, content: Any) -> dict[str, Any]:
    document = {"schema_version": 1, "manifest_type": f"u20_{name}_manifest", "content": content}
...
def execute(raw_root: Path) -> dict[str, Any]:
    ...
    qualification = load_json(ROOT / "reports/m1/evidence/u20_cross_sectional_data_qualification_v1.json")
    ...
    summary = {
        "schema_version": 1,
        "run_id": "U20-06-SEALED-IS-PAPER-OBSERVATION-V1",
        "status": status,
...
def render(result: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# U-20 Sealed-IS Paper Observation",
        "",
        ...
        *[f"- {key}: `{str(value).lower()}`" for key, value in summary["paper_gate_checks"].items()],
        ...
        f"- Median 24h relative negative-coskewness risk premium: `{metrics['median_24h_relative_negative_coskewness_risk_premium']}`",
        f"- Median 24h absolute displacement: `{metrics['median_24h_candidate_absolute_close_displacement']}`",
        f"- Positive relative-premium fraction: `{metrics['fraction_complete_episodes_with_positive_24h_relative_negative_coskewness_risk_premium']}`",
        "",
        "## Isolation",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--evidence-dir", type=Path, default=ROOT / "reports/m1/evidence/u20_cross_sectional_paper_observation")
    parser.add_argument("--report", type=Path, default=ROOT / "reports/m1/U20_CROSS_SECTIONAL_PAPER_OBSERVATION.md")
    args = parser.parse_args()
    if (args.evidence_dir / "run_manifest.json").exists():
        print("U-20 is closed after its unique Paper observation; a second result-bearing run is not authorized.")
        return 2

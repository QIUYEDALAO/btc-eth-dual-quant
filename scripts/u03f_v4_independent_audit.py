...
def fixture_smoke(order: str) -> dict:
    ...
    manifests = {
        "membership_manifest": make_manifest("audit_membership", membership),
        "qualification_summary": make_manifest(
            "audit_summary", {"months": 1, "membership_rows": 2, "authorization": {"u04": False, "m2": False}}
        ),
    }
    return {
        "mode": "fixture_only",
        ...
        "manifest_hashes": {name: item["content_hash"] for name, item in sorted(manifests.items())},

...
TARGET_SHA="03d2b8736abab277e60db1153ba73f0899d7696f"
EVIDENCE_ONLY=false
...
reviewed_draft_checks() {
  ...
  (
    cd "$temp_dir" || exit 1
    PYTHONPATH="$OLDPWD/.deps:$temp_dir/src:$temp_dir" "$PY_CMD" -m unittest tests.test_adr0015_policy_draft -v
    PYTHONPATH="$OLDPWD/.deps:$temp_dir/src:$temp_dir" "$PY_CMD" scripts/adr0015_policy_draft_check.py
  )
  result=$?
  rm -rf "$temp_dir"
  return "$result"
...
adoption_scope_scan() {
  ...
  test -z "$(printf '%s\n' "$changed" | grep -E '^(src/|config/|reports/m0/evidence/|storage/)' || true)"
}

run_check "frozen ADR-0015 Draft structured checks" reviewed_draft_checks
run_check "ADR-0015 adoption tests" "$PY_CMD" -m unittest tests.test_adr0015_adoption -v
if [[ "$EVIDENCE_ONLY" == true ]]; then
  run_check "ADR-0015 adoption manifest and evidence Gate" "$PY_CMD" scripts/adr0015_adoption_check.py --evidence-only
else
  run_check "ADR-0015 adoption manifest and repository Gate" "$PY_CMD" scripts/adr0015_adoption_check.py
fi
run_check "ADR-0015 independent-review regression" "$PY_CMD" -m unittest tests.test_adr0015_independent_review -v
run_check "ADR-0015 independent-review evidence Gate" "$PY_CMD" scripts/adr0015_independent_review_check.py
...
echo
echo "ADR-0015 adoption validation summary"

#!/usr/bin/env python3
"""Execute the one authorized U-12 sealed-IS Paper observation."""
...
from scripts.u12_cross_sectional_data_qualification import previous_month, required_tasks

PROTOCOL_TARGET = "11caf6f5160bfd03a127b6fc3565ad0b84c43d82"
PROTOCOL_PATH = "config/u12_cross_sectional_paper_protocol_v1.json"
QUALIFICATION_HASH = "c9a5b5480081ef0583604910ce2550029086b9c9f7bd0a147c723006859a0510"
QUALIFICATION_PATH = ROOT / "reports/m1/evidence/u12_cross_sectional_data_qualification_v1.json"
EVIDENCE_DIR = ROOT / "reports/m1/evidence/u12_cross_sectional_paper_observation"
...
def build_daily_returns(
    ...
    for month, symbol in ordered(tasks, order):
        for opened, _open, _high, _low, close in read_rows(raw_root, symbol, month, end):
            if opened < start - ONE_DAY_MS or (symbol, opened) in masked:
                continue
            day = (opened // ONE_DAY_MS) * ONE_DAY_MS
            counts[(symbol, day)] += 1
            if opened == day + ONE_DAY_MS - FIVE_MINUTES_MS:
                last_close[(symbol, day)] = close
    returns: dict[int, dict[str, Decimal]] = {}
    common: dict[int, Decimal] = {}
    accounting = {"daily_intervals_considered": 0, "daily_intervals_complete": 0, "daily_intervals_ineligible": 0}
    with localcontext() as context:
        context.prec = 50
        day = start
        while day < end:
            accounting["daily_intervals_considered"] += 1
            members = tuple(membership.get(month_for_ms(day), ()))
            row: dict[str, Decimal] = {}
            for symbol in members:
                current = last_close.get((symbol, day))
                prior = last_close.get((symbol, day - ONE_DAY_MS))
                if counts.get((symbol, day)) != 288 or current is None or prior is None:
                    break
                row[symbol] = (current / prior).ln()
            if len(members) >= 10 and len(row) == len(members):
                returns[day] = row
                common[day] = median(list(row.values()))
                accounting["daily_intervals_complete"] += 1
            else:
                accounting["daily_intervals_ineligible"] += 1
            day += ONE_DAY_MS
    return returns, common, accounting


def rank_top_quartile(values: Mapping[str, Decimal]) -> set[str]:
    count = (len(values) + 3) // 4
    return set(sorted(values, key=lambda symbol: (-values[symbol], symbol))[:count])


def select_events(
    daily_returns: Mapping[int, Mapping[str, Decimal]],
    ...
    accounting = {"decision_days_considered": 0, "history_ineligible": 0, "threshold_or_rank_failed": 0, "simultaneous_candidates_discarded": 0}
    decision = start + 52 * 7 * ONE_DAY_MS
    with localcontext() as context:
        context.prec = 50
        while decision + ONE_DAY_MS <= end:
            accounting["decision_days_considered"] += 1
            current = tuple(membership.get(month_for_ms(decision), ()))
            history = [decision - offset * 7 * ONE_DAY_MS for offset in range(52, 0, -1)]
            ...
            for symbol in current:
                ...
                for index, day in enumerate(history):
                    row = daily_returns.get(day)
                    if row is None or day not in common or symbol not in row:
                        continue
                    residual = row[symbol] - common[day]
                    values.append(residual)
                    (first if index < 26 else second).append(residual)
                if len(values) >= 40 and len(first) >= 18 and len(second) >= 18:
                    stats[symbol] = {"full": median(values), "first": median(first), "second": median(second), "count": len(values), "first_count": len(first), "second_count": len(second)}
            if len(stats) < 10:
                accounting["history_ineligible"] += 1
                decision += ONE_DAY_MS
                continue
            top = rank_top_quartile({symbol: row["full"] for symbol, row in stats.items()})
            candidates = [symbol for symbol in top if stats[symbol]["full"] >= Decimal("0.0060") and stats[symbol]["first"] >= Decimal("0.0030") and stats[symbol]["second"] >= Decimal("0.0030")]
            if not candidates:
                accounting["threshold_or_rank_failed"] += 1
                decision += ONE_DAY_MS
                continue
            candidates.sort(key=lambda symbol: (-stats[symbol]["full"], -min(stats[symbol]["first"], stats[symbol]["second"]), -stats[symbol]["count"], symbol))
            accounting["simultaneous_candidates_discarded"] += len(candidates) - 1
            symbol = candidates[0]
            row = stats[symbol]
            core = {
                "decision_time_ms": decision,
                "reference_open_time_ms": decision + FIVE_MINUTES_MS,
                "calendar_state": datetime.fromtimestamp(decision / 1000, tz=timezone.utc).strftime("%A").lower(),
                "symbol": symbol,
                "active_members": list(current),
                "active_member_count": len(current),
                "full_median_asset_specific_value": decimal_text(row["full"]),
                "first_half_median_asset_specific_value": decimal_text(row["first"]),
                "second_half_median_asset_specific_value": decimal_text(row["second"]),
        ...
        for event in episodes:
            ...
            for index, opened in enumerate(required, start=1):
                ...
                if candidate_value - peer_value >= Decimal("0.0030"):
                    ...
                    break
            row = dict(event)
            row.update({"candidate_absolute_close_displacement": absolute, "peer_median_close_displacement": peer_displacement, "relative_calendar_flow_persistence": relative, "candidate_mfe_through_24h": decimal_text(max(highs)), "candidate_mae_through_24h": decimal_text(min(lows)), "first_relative_recovery_to_base_cost_minutes": recovery})
            paths.append(row)
...
def evaluate_gates(paths: Sequence[Mapping[str, Any]], protocol: Mapping[str, Any], mismatch: int) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    relative = [Decimal(str(row["relative_calendar_flow_persistence"]["24"])) for row in paths]
...
def run_order(raw_root: Path, protocol: Mapping[str, Any], qualification: Mapping[str, Any], order: str) -> dict[str, Any]:
    ...
    returns, common, daily_accounting = build_daily_returns(raw_root, membership, masked, start, end, order)
    ...
    accounting = {**daily_accounting, **event_accounting, "common_component_days": len(common), "candidate_events": len(events), "independent_episodes": len(episodes), "complete_24h_episodes": len(paths), "right_censored_episodes": len(episodes) - len(paths), "right_censor_reasons": dict(sorted(censored.items())), "oos_rows_decoded": 0, "formal_returns_computed": 0, "fills_positions_or_equity_rows": 0}
...
def execute(raw_root: Path) -> dict[str, Any]:
    ...
    if protocol.get("content_hash") != "a8cfc0b74e82bdf455bae5dda7a620bc5c0c53f1022d39494800f1053dd80b8a" or qualification.get("qualification_content_hash") != QUALIFICATION_HASH:
...
def render(result: Mapping[str, Any]) -> str:
    ...
    return "\n".join(["# U-12 Sealed-IS Paper Observation", "", f"- Status: `{summary['status']}`", f"- Run hash: `{summary['run_content_hash']}`", f"- Complete independent episodes: `{metrics['complete_is_independent_episodes']}`", "", "## Frozen Paper Gates", "", *[f"- {key}: `{str(value).lower()}`" for key, value in summary["paper_gate_checks"].items()], "", "## Metrics", "", f"- Projected full / sealed OOS episodes: `{metrics['projected_full_independent_episodes']} / {metrics['projected_sealed_oos_independent_episodes']}`", f"- Median 24h relative calendar-flow persistence: `{metrics['median_24h_relative_calendar_flow_persistence']}`", f"- Median 24h candidate absolute displacement: `{metrics['median_24h_candidate_absolute_close_displacement']}`", f"- Positive relative fraction: `{metrics['fraction_complete_episodes_with_positive_24h_relative_calendar_flow_persistence']}`", f"- Failed Gates: `{', '.join(failed) if failed else 'none'}`", "", "## Isolation", "", "- OOS opened / rows decoded: `false / 0`", "- Formal returns, fills, positions or equity generated: `false`", "- Parameters changed or second run executed: `false / false`", "", "A failed Gate closes U-12 without tuning. A pass authorizes only independent Paper-result review.", ""])
...
def main() -> int:
    ...
    parser.add_argument("--report", type=Path, default=ROOT / "reports/m1/U12_CROSS_SECTIONAL_PAPER_OBSERVATION.md")
    args = parser.parse_args()
    if (args.evidence_dir / "run_manifest.json").exists():
        print("U-12 result already exists; a second result-bearing run is prohibited.", file=sys.stderr)
        return 2

...
read_only_scan() { local order="order" post="PO""ST" delete_word="DE""LETE" fill="fill" engine="engine"; local pattern="(/api/v3/${order}|/fapi/v1/${order}|/sapi/v1/margin/${order}|${post}|${delete_word}|place_${order}|cancel_${order}|create_${order}|simulate_${fill}|matching_${engine}|freqtrade[[:space:]]+trade)"; ! rg -n "$pattern" scripts/u13_cross_sectional_data_qualification.py scripts/u13_cross_sectional_data_qualification_check.py; }
python3 scripts/u13_cross_sectional_data_qualification_check.py
python3 -m unittest -v tests.test_u13_cross_sectional_data_qualification
python3 scripts/u13_cross_sectional_paper_protocol_review_check.py

...
from scripts.u23_cross_sectional_paper_observation_check import RUN,validate
class U23ObservationTests(unittest.TestCase):
 ...
 def test_result_or_second_run_tamper(self):
  x=copy.deepcopy(self.run);x["metrics"]["complete_is_independent_episodes"]=179;self.assertTrue(validate(x))

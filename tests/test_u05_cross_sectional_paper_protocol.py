...
from scripts.u05_cross_sectional_paper_protocol_check import EXPECTED_HASH, PROTOCOL_PATH, load, validate, validate_report


class U05CrossSectionalPaperProtocolTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_every_authority_binding_is_hash_bound(self):
        for key in self.protocol["authority_bindings"]:
            changed = copy.deepcopy(self.protocol); changed["authority_bindings"][key] = "0" * 64
    ...
    def test_range_alignment_and_oos_cannot_change(self):
        for key, value in (("is_start", "2021-01-01T00:00:00Z"), ("signal_timeframe", "1h"), ("signal_alignment", "rolling"), ("oos_opened", True)):
            changed = copy.deepcopy(self.protocol); changed["scope"][key] = value
            self.assertTrue(validate(changed), key)

    def test_member_breadth_common_move_cluster_or_horizon_change_fails(self):
        mutations = [
            ("point_in_time_universe", "minimum_active_members", 9),
            ("breadth_event", "breadth_fraction_minimum", "0.79"),
            ("breadth_event", "common_move_minimum", "0.0119"),
            ("episode_clustering", "connected_window_hours", 23),
            ("observation_contract", "horizon_hours", [1, 4, 24]),
        ]
        for section, key, value in mutations:
            changed = copy.deepcopy(self.protocol); changed[section][key] = value
            self.assertTrue(validate(changed), f"{section}.{key}")
    ...
    def test_membership_lifecycle_or_later_search_shortcut_fails(self):
        for key in ("replacement_member_allowed", "current_membership_backfill_allowed", "lifecycle_crossing_assumption_allowed"):
            changed = copy.deepcopy(self.protocol); changed["point_in_time_universe"][key] = True
            self.assertTrue(validate(changed), key)
        changed = copy.deepcopy(self.protocol); changed["observation_contract"]["search_later_if_reference_missing"] = True
    ...
    def test_gate_or_cost_reduction_fails(self):
        ...
        self.assertTrue(validate(changed))
        changed = copy.deepcopy(self.protocol); changed["cost_reference_roundtrip"]["cost_x2"] = "0.0059"
        self.assertTrue(validate(changed))
    ...
    def test_extra_result_or_fixed_rule_field_fails(self):
        for key in ("target", "stop", "position", "returns", "event_count", "outcome"):
            changed = copy.deepcopy(self.protocol); changed[key] = "forbidden"
            self.assertTrue(validate(changed), key)

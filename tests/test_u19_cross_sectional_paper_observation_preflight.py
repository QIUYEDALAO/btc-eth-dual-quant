...
from scripts.u19_cross_sectional_data_qualification import HIGH_HALF, LOW_HALF
from scripts.u19_cross_sectional_paper_observation import ONE_HOUR_MS, evaluate_gates, main, observe_paths, select_events
...
def path(decision: int, symbol: str, relative: str = "0.020", absolute: str = "0.020") -> dict:
    return {
        ...
        "symbol": symbol,
        "relative_volatility_of_volatility_risk_premium": {"24": relative},
        "candidate_absolute_close_displacement": {"24": absolute},
    }


class U19ObservationPreflightTests(unittest.TestCase):
    def test_complete_prior_history_selects_cross_sectional_representative(self) -> None:
        ...
        for symbol_index, symbol in enumerate(symbols):
            history = (HIGH_HALF + HIGH_HALF) if symbol_index < 4 else (LOW_HALF + LOW_HALF)
            value = 100.0
    ...
    def test_failed_economic_gate_does_not_authorize(self) -> None:
        ...
        self.assertFalse(checks["median_24h_relative_volatility_of_volatility_risk_premium"])
    ...
    def test_observation_path_uses_reference_open_and_all_peers(self) -> None:
        ...
        self.assertEqual(paths[0]["relative_volatility_of_volatility_risk_premium"]["24"], "0.02")
    ...
    def test_second_run_guard(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            original = sys.argv
            try:
                sys.argv = ["u19", "--evidence-dir", str(evidence), "--report", str(evidence / "report.md")]

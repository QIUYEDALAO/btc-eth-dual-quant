...
from scripts.u19_cross_sectional_paper_observation_check import RUN, validate


class U19ObservationTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_result_or_order_tamper_fails(self) -> None:
        ...
        changed["metrics"]["complete_is_independent_episodes"] = 55
    ...
    def test_failed_gate_or_permission_tamper_fails(self) -> None:
        changed = copy.deepcopy(self.run)
        changed["paper_gate_checks"]["median_24h_relative_volatility_of_volatility_risk_premium"] = True
        self.assertTrue(validate(changed))
        changed = copy.deepcopy(self.run)

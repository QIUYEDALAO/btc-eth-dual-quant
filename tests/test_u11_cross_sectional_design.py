...
from scripts.u11_cross_sectional_design_check import CONTENT_HASH, SCOPE, load, validate_scope


class U11DesignTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_permission_escalation_fails(self):
        for key in ("event_scan", "returns", "fixed_rule_contract", "backtesting", "oos", "api_trading", "execution_live", "m2"):
            changed = copy.deepcopy(self.document)
    ...
    def test_removing_invariant_or_failure_fails(self):
        changed = copy.deepcopy(self.document)
        changed["causal_and_membership_invariants"].pop("positive_and_negative_state_estimators_use_only_prior_completed_observations")

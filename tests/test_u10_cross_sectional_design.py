...
from scripts.u10_cross_sectional_design_check import CONTENT_HASH,SCOPE,load,validate_scope
class U10DesignTests(unittest.TestCase):
 ...
 def test_removing_invariant_or_failure_fails(self):
  d=copy.deepcopy(self.d);d["causal_and_membership_invariants"].pop("quote_volume_share_denominator_uses_all_complete_active_members");self.assertTrue(validate_scope(d))
  d=copy.deepcopy(self.d);d["economic_hypothesis"]["failure_regimes"].pop();self.assertTrue(validate_scope(d))
 def test_resolving_parameter_prematurely_fails(self):d=copy.deepcopy(self.d);d["unresolved_until_separate_protocol"].pop();self.assertTrue(validate_scope(d))

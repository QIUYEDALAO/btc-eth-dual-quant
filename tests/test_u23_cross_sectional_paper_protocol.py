...
from scripts.u23_cross_sectional_paper_protocol_check import FEASIBILITY, PROTOCOL, validate_feasibility, validate_protocol


class U23ProtocolTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_parameter_or_gate_tamper(self):
        changed = copy.deepcopy(self.protocol)
        changed["range_contract"]["minimum_current_over_baseline_ratio"] = "1.90"
        self.assertTrue(validate_protocol(changed))
        changed = copy.deepcopy(self.protocol)
        changed["paper_gates"]["combined_median_24h_relative_close_strength_continuation_minimum"] = "0.0100"
        self.assertTrue(validate_protocol(changed))

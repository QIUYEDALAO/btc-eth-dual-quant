...
from scripts.u13_cross_sectional_paper_protocol_review_check import EVIDENCE, REPORT, validate


class U13ProtocolReviewTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_target_or_verdict_tamper_fails(self):
        for key, value in (("target_commit", "0" * 40), ("remaining_high_findings", 1), ("target_modified", True)):
            changed = copy.deepcopy(self.document); changed[key] = value
            self.assertTrue(validate(changed, self.report), key)

    def test_dimension_or_permission_escalation_fails(self):
        changed = copy.deepcopy(self.document); changed["dimensions"]["prior_only_2160h_lag_history"] = "fail"
        self.assertTrue(validate(changed, self.report))
        changed = copy.deepcopy(self.document); changed["authorizations"]["event_scan"] = True
        self.assertTrue(validate(changed, self.report))

...
from scripts.u24_cross_sectional_design_check import CONTENT_HASH,SCOPE,validate_scope
class U24DesignTests(unittest.TestCase):
 ...
 def test_escalation_or_parameter_fails(self):
  changed=copy.deepcopy(self.scope);changed["authorizations"]["event_scan"]=True;self.assertTrue(validate_scope(changed))
  changed=copy.deepcopy(self.scope);changed["unresolved_until_separate_protocol"].pop();self.assertTrue(validate_scope(changed))
 def test_failure_or_nonduplication_removal_fails(self):
  ...
  changed=copy.deepcopy(self.scope);changed["non_duplication"]["u23_range_expansion_close_strength_continuation_prohibited"]=False;self.assertTrue(validate_scope(changed))

...
import json
import unittest
from scripts.u07_cross_sectional_design_check import SCOPE_PATH, validate_scope, validate_ledger, validate_bound_evidence


class U07CrossSectionalDesignTests(unittest.TestCase):
    ...
    def test_prior_result_or_authorization_tamper_fails(self):
        for key in ("u07_design_authorization_hash", "u06_closed_run_hash", "u05_closed_run_hash", "u04_closed_run_hash"):
    ...
    def test_causal_nonduplication_or_failure_regime_removal_fails(self):
        ...
        changed = copy.deepcopy(self.scope); changed["non_duplication"].pop("m1c_unconditional_long_cycle_winner_rotation_reuse_prohibited"); self.assertTrue(validate_scope(changed))
    ...
    def test_parameter_or_permission_escalation_fails(self):
        changed = copy.deepcopy(self.scope); changed["selected_timeframe"] = "4h"; self.assertTrue(validate_scope(changed))

...
| U-03E-ADJ | Liquid universe V2 source conflict adjudication | in_progress | U-03E truthful blocked evidence merged | `codex/u03e-source-conflict-adjudication` / PR pending | Checksum-bound raw-row, daily ZIP and two-host public REST evidence | All three conflicts classified; report exactly regenerates; no contract bypass | Decision is `new_policy_adr_required`; same-contract rerun remains unauthorized pending a separately reviewed ADR |
...
| U-04 | Cross-sectional hypothesis design | not_authorized | U-03F merged pass and separate task | not created | One preregistered cross-sectional hypothesis | Design only, no outcomes or OOS | V2 public requalification is blocked and independent audit has not run |

## Current Gate

- Authorized work: review U-03E adjudication evidence, then prepare a separate Draft data-policy ADR only.

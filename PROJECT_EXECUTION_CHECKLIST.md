...
| T5-01 | M1D feasibility | blocked | T4-01 completed | `codex/short-horizon-t5-sample-budget-precheck` / PR pending | Metadata-only M1D sample-budget precheck | Exact manifests, sealed OOS, fixed 70/30 and 540-day Gate, no return access | 302 OOS days < fixed 540-day minimum; earliest projected resolution 2028-09-03 |
...
- Authorized work: finish the T5 failure-record PR/CI/merge and governance closeout only.
- Active implementation: T5 metadata-only precheck completed; candidate evaluated `no`, events selected `no`, OOS prices/returns accessed `no`.

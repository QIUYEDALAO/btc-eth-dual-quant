...
- Current phase: the checksum-bound KLAYUSDT source adjudication is complete pending review; it classifies the row as `symbol_lifecycle_boundary_artifact` and requires a separate new-policy ADR. No policy change, registry revision, V3 rerun or strategy is authorized.
...
- U-03F must not start while V3 is blocked. U-04 remains unauthorized. U-03E-V3-ADJ changed no contract, registry or qualification artifact and ran no V3 build. A new-policy ADR is only a future candidate after review and explicit authorization.

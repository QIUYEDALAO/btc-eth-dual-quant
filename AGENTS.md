...
- U-04 paper protocol `7b0e462dd9d4f51de1419005bb8701b859f4d2be6148121c1e68cdd0089629d6` is frozen before results with exact 1h median/MAD, -3.0/-1.80% event, 24h clustering, path observations and paper Gates. No public data, event, path, return or OOS value was read.
- Only an exact-head independent review of that frozen protocol is authorized next. Data qualification and protocol execution remain unauthorized until a separate approving review.
...
- The only current successor is the U-04 outcome-blind paper-protocol design. The protocol must be frozen before any event scan, and protocol completion alone does not authorize execution.
